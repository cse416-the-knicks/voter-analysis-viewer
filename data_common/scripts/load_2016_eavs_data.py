...
eavs_df["mail_reject_total"] = eavs_df[["C4b","B13a"]].sum(axis=1, skipna=True, min_count=1)

# Compute removed_other as A11h + A11i + A11j + A11k (skipping NaN)
eavs_df["removed_other"] = eavs_df[["A11h","A11i","A11j","A11k"]].sum(axis=1, skipna=True, min_count=1)

# Compute prov_other as E2j + E2k + E2l + E2m + E2n + E2o + E2p (skipping NaN)
eavs_df["prov_other"] = eavs_df[["E2j","E2k","E2l","E2m","E2n","E2o","E2p"]].sum(axis=1, skipna=True, min_count=1)

# Compute mail_reject_other as C5o + C5p + C5q + C5r + C5s + C5t + C5u + C5v (skipping NaN)
eavs_df["mail_reject_other"] = eavs_df[["C5o","C5p","C5q","C5r","C5s","C5t","C5u","C5v"]].sum(axis=1, skipna=True, min_count=1)

# Compute total_ballots_cast as the sum of absentee, early, eday, and provisional
eavs_df["total_ballots_cast"] = eavs_df[["C4a","B8a","F1f","F1b","E1a"]].sum(axis=1, skipna=True, min_count=1)
...
eavs_df.drop(eavs_df[eavs_df["State"] == "AS"].index, inplace=True)

# Dropping the unused other columns before writing
eavs_df = eavs_df.drop(columns=["A11h","A11i","A11j","A11k","E2j","E2k","E2l","E2m","E2n","E2o","E2p","C5o","C5p","C5q","C5r","C5s","C5t","C5u","C5v","State","B13a","C4b","B8a"])
