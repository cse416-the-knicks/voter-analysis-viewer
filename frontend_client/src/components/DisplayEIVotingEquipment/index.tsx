...
import type { EIXYPoint } from "../../api/client";
          ...
          {data.map((x) => (
            <>
              <span style={{ paddingLeft: "8px", paddingRight: "8px", paddingBottom: "0", margin: "auto", height: "5px", display: "flex" }}>
                <i style={{ background: x.fillColor, width: "18px", height: "18px", display: "inline-block", marginRight: "8px", border: "1.5px solid black" }}></i>
  ...
  return (
    <>
      <svg ref={svgRef} width={width} height={height} style={{ background: "#ffff", borderRadius: "8px" }}>
        ...
        {/* PDF Curves */}
        {
          actualData.map((x) => <path data-title={x.title} d={curveLine(x.samples)} opacity={x.opacity ?? "0.5"} fill={x.fillColor} stroke={x.strokeColor ?? "black"}/>)
        }
      </svg>
