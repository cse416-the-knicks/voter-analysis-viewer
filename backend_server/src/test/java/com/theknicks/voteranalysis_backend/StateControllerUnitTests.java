...
public class StateControllerUnitTests {
  @Autowired private MockMvc _mvc;

  @MockitoBean private StateService service;

  void TestSetup_GetBoundaryGeometry(Optional<ObjectNode> result) {
    Mockito.when(service.getBoundaryGeometry(ArgumentMatchers.anyString())).thenReturn(result);
  }
  ...
  void StateController_getStateGeometry_ResponseSuccess() throws Exception {
    ...
    // Act
    _mvc.perform(MockMvcRequestBuilders.get("/state/{fipsCode}/geometry", "1"))
        // Assert
  ...
  void StateController_getStateGeometry_ResponseInvalid() throws Exception {
    ...
    // Act
    _mvc.perform(MockMvcRequestBuilders.get("/state/{fipsCode}/geometry", "3.141592654"))
        // Assert
        .andExpect(status().isOk())
