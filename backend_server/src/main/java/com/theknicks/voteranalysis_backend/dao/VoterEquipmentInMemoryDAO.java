...
/**
 * NOTE(jerry): This DAO exists so that it is possible to unit test the logic of this DAO, because
 * otherwise the tests would need to read from disk, which is not optimal.
 *
 * <p>This happens to be because JDBC doesn't need to be used for this since it requires so little
...
public class VoterEquipmentInMemoryDAO implements IVoterEquipmentDAO {
  ...
  public List<VotingEquipmentModel> getVotingEquipmentByType(String type) {
    return _equipmentList.stream().filter((item) -> item.equipmentType().equals(type)).toList();
  }

  @Override
  public List<VotingEquipmentModel> getVotingEquipmentByManufacturer(String manufacturer) {
    return _equipmentList.stream()
        .filter((item) -> item.manufacturer().equals(manufacturer))
        .toList();
  }
